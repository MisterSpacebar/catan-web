...
export function ResourceChip({ resource, count, showLabel = false, size = "md", className }) {
  ...
  return (
    <div
      className={cn(
        "inline-flex items-center rounded-full shadow-md",
        sizeClasses[size],
        className
      )}
      style={{
    ...
    >
      <span className={size === "sm" ? "text-[12px]" : size === "lg" ? "text-[16px]" : "text-[14px]"}>
        {emoji}
      </span>
...
export function ResourceSummary({ resources, className }) {
  ...
  return (
    <div className={cn("flex items-center gap-1", className)}>
      {entries.length === 0 ? (
        <span className="text-[11px] text-slate-600">No resources</span>
      ) : (
        entries.map(([resource, count]) => (
          <span key={resource} className="text-[13px]" title={`${getResourceLabel(resource)}: ${count}`}>
            {getResourceEmoji(resource)}
          </span>
        ))
      )}
      <span className="text-[11px] text-slate-500 ml-1">({total})</span>
    </div>
  );
}
